# Universal AI-DLC principles

The universal route makes code changes evidence-backed without turning every
task into ceremony:
...
3. **Adaptive but explicit depth.** Conditional stages are completed or
   factually skipped. A skip never creates a hidden dependency or a pause.
4. **One approval boundary.** 1.7 approves, re-plans, or declines the plan.
   Later material changes are re-planned from their owning decision.
5. **Architecture before generation.** Requirements, application boundaries,
...
The route contains Initialization, Ideation, Inception, and Construction only.
