...
export const renderAidlcIntent = (intent: AidlcIntent): string =>
  matter.stringify(
    [
      ...
      '## Research',
      '',
      'Record the research question, named evidence sources, observed facts, inferences, existing proof, risks, and remaining uncertainty. A blank section is not a completed handoff; use an evidence-backed `N/A` only when the section is genuinely inapplicable.',
      '',
      '## Decisions',
      '',
      'Record requirements and preservation constraints, the selected approach, consequences, rejected alternatives, reversibility, and any unresolved blocker or re-plan trigger. Mark proposed versus approved decisions explicitly.',
      '',
      '## Plan',
      '',
      'Record the component or module design, owner boundaries, dependencies, requirements-to-plan mapping, and the ordered construction plan below. Each increment must name what changes, where, why this order is chosen, focused proof, risks, and its re-plan trigger.',
      '',
      ...
      '## Validation evidence',
      '',
      'Map every acceptance item and material claim to focused proof and its relation to the configured final gate. Record actual commands, observed results, limitations, and any open follow-up; a passing final gate does not prove an unmapped criterion.',
      '',
      '## Outcome',
      '',
      '## Audit trail',
